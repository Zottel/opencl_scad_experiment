...
enum ${NAME}_INSTATE {
	${NAME}_INSTATE_INVALID = 0, ${NAME}_INSTATE_SYNC = 1, ${NAME}_INSTATE_OPC = 2, ${NAME}_INSTATE_ADDRESS = 3, ${NAME}_INSTATE_DATA = 4
};
...
__kernel void ${NAME}_external_input() {
	...
	enum ${NAME}_INSTATE state = ${NAME}_INSTATE_SYNC;
	
	while(true) {
		scad_input_handle(${NUMBER}, &input_manage, input);
		
		
		if(state == ${NAME}_INSTATE_SYNC){
			if(buffer_input_has_data(&input[2])) { // includes marker
				bool sync_val = buffer_input_has_marker(&input[2]);
				if(write_channel_nb_altera(${NAME}_channel_input_sync, sync_val)) {
					if(sync_val) {
...
__kernel void ${NAME}(__global scad_data * restrict mem,
                      cl_uint mem_length) {

#ifdef EMULATOR
	printf("[${NAME}] starting with id ${NUMBER}\n");
#endif
	while(!read_channel_altera(${NAME}_channel_input_sync)) {
		#ifdef EMULATOR
